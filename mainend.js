...
const firebaseConfig = {
    ...
    storageBucket: "group-i-77569.firebasestorage.app",
...
window.onload = function() {
    db.collection('class').orderBy('timestamp', 'asc') // 日時で昇順にソート
    .onSnapshot(snapshot => {
        const messages = [];
...
//             const timestamp = data.timestamp ? data.timestamp.toDate().toLocaleString() : ""; // タイムスタンプを日時に変換
//             if (data.user && data.message) { // undefinedのフィールドを除外
//                 messages.push('<li>日時: ' + timestamp + '<br>ユーザー名: ' + data.user + '<br>内容: ' + data.message + '</li>');
//             }
//         });
